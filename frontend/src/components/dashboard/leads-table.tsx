  ...
  return (
    <div className="max-h-[420px] overflow-auto border-t border-slate-200">
      <Table>
        ...
        <TableBody>
          {leads.map((lead) => (
            <TableRow key={lead._id} className="h-12 cursor-pointer" onClick={() => onRowClick(lead)}>
              <TableCell className="text-sm font-medium text-slate-900">{lead.name}</TableCell>
              <TableCell className="text-sm text-slate-700">{lead.email}</TableCell>
              <TableCell><Badge className="text-[11px] uppercase" variant={statusVariant[lead.status]}>{lead.status}</Badge></TableCell>
              <TableCell className="text-sm text-slate-700"><div className="flex items-center gap-1.5">{sourceIcon(lead.source)}{lead.source}</div></TableCell>
              <TableCell className="text-sm text-slate-700">{new Date(lead.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</TableCell>
