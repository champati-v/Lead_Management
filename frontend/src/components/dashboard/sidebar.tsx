...
export function Sidebar({ user, open, onClose, onCreateLead, onLogout, onViewProfile }: SidebarProps) {
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-60 flex-col border-r border-slate-200 bg-white px-4 py-4 transition-transform md:static md:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
      <div className="mb-6 flex items-center gap-2.5">
        <div className="grid h-8 w-8 place-items-center rounded-md bg-[#0d57d5] text-white"><LayoutList className="h-4 w-4" /></div>
        <div>
          <p className="text-base font-semibold text-slate-900">Smart Leads</p>
      ...
      <button className="mt-3 flex h-9 w-full items-center gap-2 rounded-md bg-blue-50 px-3 text-sm font-medium text-blue-700"><Users className="h-4 w-4" />Leads</button>

      <div className="mt-auto">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="flex w-full items-center gap-2 rounded-md border border-slate-200 px-2.5 py-2 text-left">
              <Avatar className="h-7 w-7"><AvatarFallback className="text-xs">{avatarInitial}</AvatarFallback></Avatar>
              <div className="min-w-0"><p className="truncate text-sm text-slate-800">{profileText}</p><p className="text-xs capitalize text-slate-500">{user.role}</p></div>
            </button>
          </DropdownMenuTrigger>
